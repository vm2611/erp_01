<?php
require_once 'config.php'; // Incluindo o arquivo de configuração correto

// Buscar os produtos para preencher o campo de seleção
try {
    $sql = "SELECT produto_id, nome FROM produtos"; // Consulta para buscar produtos
    $stmt = $conn->prepare($sql);
    $stmt->execute();
    $products = $stmt->fetchAll(PDO::FETCH_ASSOC); // Guardar os produtos em uma variável
} catch (PDOException $e) {
    echo "Erro ao buscar produtos: " . $e->getMessage();
    $products = []; // Caso ocorra um erro, inicializa como um array vazio
}

// Buscar as movimentações no banco de dados
try {
    $sql = "SELECT e.estoque_id, p.nome AS produto_nome, e.tipo_movimentacao, e.quantidade, e.data_movimentacao
            FROM estoque e
            JOIN produtos p ON e.produto_id = p.produto_id
            ORDER BY e.data_movimentacao DESC"; // Ajuste o campo conforme necessário
    $stmt = $conn->prepare($sql);
    $stmt->execute();
    $movimentacoes = $stmt->fetchAll(PDO::FETCH_ASSOC); // Definindo a variável $movimentacoes
} catch (PDOException $e) {
    echo "Erro ao buscar movimentações: " . $e->getMessage();
    $movimentacoes = []; // Caso ocorra um erro, a variável será inicializada como um array vazio
}
